...
本目录只保存重绘语言验证 worker 的 systemd unit 和发布说明。worker source 和 venv 必须预置在 `/opt/moli-drama/shared/redraw-locale-verifier/`；本 release 只携带 sandbox materials、systemd unit 和 release scope，不携带源码、模型权重、虚拟环境、密钥或生产数据。
