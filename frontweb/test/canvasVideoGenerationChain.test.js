...
test('画布视频节点把模型、首尾帧和素材引用传给视频创建接口', () => {
  ...
  assert.match(runnerSource, /const voiceSnapshot = buildStoryboardVoiceSnapshot\(drama,\s*sb\)/)
  assert.match(runnerSource, /const voiceCharacters = voiceSnapshot\.characters/)
  ...
  assert.match(runnerSource, /fetchAssignedAssetReferences\(sb\.id\)/)
  assert.match(runnerSource, /referenceKind\(a\?\.type \|\| a\?\.mime_type, url\)/)
  assert.match(runnerSource, /function upstreamReferenceUrls\(genOpts = \{\}\)/)
  assert.match(runnerSource, /async function autoLinkTailFrameAfterVideo\(drama, storyboard, found\)/)
  assert.match(runnerSource, /canChainStoryboardFrames\(next, current\)/)
  assert.match(runnerSource, /storyboardsAPI\.linkTailFrame\(current\.id, \{ drama_id: drama\.id \}\)/)
  assert.match(runnerSource, /const typedUpstreamRefs = upstreamReferenceItems\(genOpts\)/)
  assert.match(runnerSource, /if \(model && !catalogEntry && !suppliedCapability/)
  assert.match(runnerSource, /getStoryboardVideoModel\(sb,\s*genOpts\)/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*firstFrameUrl:\s*referenceMode === 'omni' \? undefined : absoluteFirst/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*lastFrameUrl:\s*referenceMode === 'omni' \? undefined : absoluteLast/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*referenceImageUrls:\s*referenceMode === 'first_last' \? undefined : referenceImageUrls/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*referenceVideoUrls:\s*referenceMode === 'first_last' \? undefined : referenceVideoUrls/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*referenceAudioUrls:\s*referenceMode === 'first_last' \? undefined : referenceAudioUrls/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*capability,[\s\S]*referenceMode,[\s\S]*generateAudio:/)
  assert.match(runnerSource, /buildVideoGenerationRequest\(\{[\s\S]*resolution:\s*genOpts\.videoResolution/)
  assert.match(runnerSource, /const requestAudit = buildVideoGenerationAudit\(\{[\s\S]*payload,[\s\S]*voicePolicy,[\s\S]*voicePrompt: voicePromptPreview,[\s\S]*voiceSnapshot/)
  assert.match(runnerSource, /const tailFrameResult = genOpts\.autoLinkTailFrame === false[\s\S]*autoLinkTailFrameAfterVideo\(drama, sb, found\)/)
  assert.match(runnerSource, /tailFrameLinked: true/)
  assert.match(runnerSource, /tailFrameLinkError/)
  assert.match(runnerSource, /nextStep: 'audio'/)
  assert.match(runnerSource, /videosAPI\.create\(payload\)/)
})

test('普通画布视频引用保持素材类型与完整列表，由模型能力明确拒绝超限', () => {
  assert.doesNotMatch(storyboardPanelSource, /allReferenceAssets\s*=\s*computed\([^\n]*\.slice\(0,\s*10\)/)
  assert.doesNotMatch(storyboardPanelSource, /assignedAssets\.value\s*=\s*\[[^\n]*\.slice\(0,\s*10\)/)
  assert.match(runnerSource, /const assignedRefs = await fetchAssignedAssetReferences\(sb\.id\)/)
  assert.doesNotMatch(runnerSource, /typedAssignedRefs = assignedRefs\.map/)
})
...
test('画布真实模型链路把 task_id 和轮询状态暴露给节点队列', () => {
  ...
  assert.match(runnerSource, /return \{[\s\S]*taskId: res\.task_id,[\s\S]*requestPayload: payload,[\s\S]*requestAudit,[\s\S]*\.\.\.\(tailFrameResult \|\| \{\}\),[\s\S]*task: polled/)
