...
test('右键画布支持上传和粘贴素材并按点击位置落点', () => {
  ...
  assert.match(uploadFunctionSource, /const uploadedAsset = await uploadAPI\.uploadMedia\(file, \{ dramaId: drama\.value\.id \}\)/)
  assert.match(uploadFunctionSource, /if \(!uploadedAsset\?\.id\) throw new Error\('素材上传成功但未返回资产记录'\)/)
  assert.match(uploadFunctionSource, /const asset = await markProjectAssetForCanvas\(uploadedAsset, 'canvas_context_upload'\)/)
