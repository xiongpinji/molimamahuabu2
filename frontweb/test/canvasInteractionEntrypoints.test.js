...
test('右键素材库在画布内弹窗选用并定位项目素材节点', () => {
  ...
  assert.match(canvasSource, /nodeStatus\.success\(nodeId, \{[\s\S]*canvasAssetStatusPayload\(projectAsset \|\| asset, \{[\s\S]*message: resultMessage,[\s\S]*attachedSlot: targetStoryboardId \? assetAttachSlot\(projectAsset \|\| asset\) : '',[\s\S]*attachedToStoryboardId: targetStoryboardId/)
  assert.match(canvasSource, /nodeStatus\.fail\(nodeId, \{[\s\S]*canvasAssetStatusPayload\(projectAsset \|\| asset, \{[\s\S]*errorDetail: message,[\s\S]*attachedToStoryboardId: targetStoryboardId/)
