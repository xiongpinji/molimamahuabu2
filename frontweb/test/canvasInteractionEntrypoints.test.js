...
test('画布支持手动节点连线并持久化到布局', () => {
  ...
  assert.match(canvasSource, /function cutCanvasEdges\(edgeIds = \[\], source = 'remove'\)/)
  assert.match(canvasSource, /if \(cutCanvasEdges\(removedIds\)\) ElMessage\.success\('已删除画布连线'\)/)
  assert.match(canvasSource, /ElMessage\.success\('已删除画布连线'\)/)
  assert.match(canvasSource, /scheduleLayoutSave\(\)/)
  assert.match(canvasSource, /buildCanvasLayoutPayload\(\s*allGraphNodes\.value,\s*currentViewport\.value,\s*layoutCache\.value,\s*allGraphEdges\.value,\s*\[\.\.\.suppressedEdgeIds\.value\],?\s*\)/)
})
