...
test('画布保留 LibTV 式导航、框选和拖拽历史入口', () => {
  ...
  assert.match(canvasSource, /if \(!event\.ctrlKey && !event\.metaKey\) return/)
...
test('节点拖拽停止后立即刷新布局缓存并同步视口', () => {
  assert.match(canvasSource, /function refreshLayoutCacheFromGraph\(\) \{[\s\S]*layoutCache\.value = withCanvasPersistedState\(buildCanvasLayoutPayload\([\s\S]*allGraphNodes\.value,[\s\S]*currentViewport\.value,[\s\S]*layoutCache\.value,[\s\S]*allGraphEdges\.value,/)
...
test('右键空白画布提供 LibTV 式添加节点入口并使用点击位置', () => {
  ...
  assert.match(contextMenuSource, /label: '3D 导演台'/)
  assert.match(contextMenuSource, /机位与角色调度/)
...
test('右键素材库在画布内弹窗选用并定位项目素材节点', () => {
  ...
  assert.match(canvasSource, /import \{ characterAPI \} from '@\/api\/characters'/)
  assert.match(canvasSource, /function storyboardCharacterIds\(storyboard\)/)
  assert.match(canvasSource, /function voiceCatalogBindId\(asset\)/)
  assert.match(canvasSource, /metadata\.voice_catalog_id/)
  assert.match(canvasSource, /asset\?\.category === 'voice' && projectAssetId\(asset\) \? `asset-\$\{projectAssetId\(asset\)\}` : ''/)
  assert.match(canvasSource, /async function bindVoiceAssetToSingleStoryboardCharacter\(asset, storyboardId, \{ silent = false \} = \{\}\)/)
  assert.match(canvasSource, /normalizePickedAssetType\(asset\) !== 'audio'/)
  assert.match(canvasSource, /findStoryboardInDrama\(drama\.value, storyboardId\)\?\.storyboard/)
  assert.match(canvasSource, /characterAPI\.bindVoiceCatalog\(characterIds\[0\], bindId, \{ silentError: true \}\)/)
  assert.match(canvasSource, /const voiceBind = await bindVoiceAssetToSingleStoryboardCharacter\(assignedAsset, storyboardId, \{ silent \}\)/)
  assert.match(canvasSource, /voiceBind\?\.bound \? '已指派音色并绑定到分镜角色' : '已指派素材并设为分镜音频'/)
...
test('右键画布支持上传和粘贴素材并按点击位置落点', () => {
  ...
  assert.match(canvasSource, /const CANVAS_MEDIA_ACCEPT = '\.png,\.jpg,\.jpeg,\.gif,\.webp,\.mp4,\.mov,\.m4a,\.m4b,\.webm,\.wav,\.mp3,\.ogg,\.oga,\.flac,\.aac'/)
  assert.match(canvasSource, /const CANVAS_IMAGE_ACCEPT = '\.png,\.jpg,\.jpeg,\.gif,\.webp'/)
  assert.match(canvasSource, /const CANVAS_VIDEO_ACCEPT = '\.mp4,\.mov,\.webm'/)
  assert.match(canvasSource, /const CANVAS_AUDIO_ACCEPT = '\.m4a,\.m4b,\.webm,\.wav,\.mp3,\.ogg,\.oga,\.flac,\.aac'/)
  assert.match(canvasSource, /const canvasUploadAccept = ref\(CANVAS_MEDIA_ACCEPT\)/)
  assert.match(canvasSource, /function openCanvasUpload\(flowPosition = null, accept = CANVAS_MEDIA_ACCEPT\)/)
  assert.match(canvasSource, /canvasUploadFlowPos\.value = flowPosition/)
  assert.match(canvasSource, /canvasUploadAccept\.value = accept \|\| CANVAS_MEDIA_ACCEPT/)
  ...
  const uploadFunctionStart = canvasSource.indexOf('async function createCanvasProjectAssetFromUpload')
  const uploadFunctionEnd = canvasSource.indexOf('\nasync function onCanvasUpload', uploadFunctionStart)
  const uploadFunctionSource = canvasSource.slice(uploadFunctionStart, uploadFunctionEnd)
  assert.match(uploadFunctionSource, /const asset = await uploadAPI\.uploadMedia\(file, \{ dramaId: drama\.value\.id \}\)/)
  assert.match(uploadFunctionSource, /if \(!asset\?\.id\) throw new Error\('素材上传成功但未返回资产记录'\)/)
  assert.doesNotMatch(uploadFunctionSource, /assetsAPI\.create/)
  ...
  assert.match(canvasSource, /if \(type === 'upload-image'\) \{[\s\S]*openCanvasUpload\(flowPosition, CANVAS_IMAGE_ACCEPT\)/)
  assert.match(canvasSource, /if \(type === 'upload-video'\) \{[\s\S]*openCanvasUpload\(flowPosition, CANVAS_VIDEO_ACCEPT\)/)
  assert.match(canvasSource, /if \(type === 'upload-audio'\) \{[\s\S]*openCanvasUpload\(flowPosition, CANVAS_AUDIO_ACCEPT\)/)
  assert.match(canvasSource, /canvasUploadAccept\.value = CANVAS_MEDIA_ACCEPT/)
...
test('右键节点支持追加下游分镜并自动创建手动连线', () => {
  ...
  assert.match(canvasSource, /allGraphEdges\.value = decorateCanvasEdges\(\[\.\.\.allGraphEdges\.value, edge\]\)/)
  assert.match(canvasSource, /await persistCanvasState\(\{ layoutOnly: true \}\)/)
  assert.match(canvasSource, /await refreshCanvas\(false\)\s*\n\s*focusedNodeId\.value = null\s*\n\s*scheduleVirtualization\(\)/)
})
