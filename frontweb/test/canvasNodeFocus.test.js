...
test('节点定位通过 setCenter 居中远距离节点并保留 fitView 回退', () => {
  ...
  assert.match(canvasSource, /async function focusCanvasNode\(nodeId\) \{[\s\S]*const node = findGraphNode\(nodeId\)[\s\S]*api\.setCenter\(centerX, centerY, \{ zoom, duration: 320 \}\)/)
