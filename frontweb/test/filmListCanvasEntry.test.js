...
test('首页单次生成由后端模型目录驱动且一键进入真实生成', () => {
  ...
  assert.match(filmListSource, /v-for="value in homeAspectRatioOptions"/)
  assert.match(filmListSource, /homeReferenceRequired/)
...
test('独立创作页承接文字、图片和视频真实生成且统一刷新积分', () => {
  ...
  assert.match(freeCreateSource, /v-for="value in selectedAspectRatioOptions"/)
  assert.match(freeCreateSource, /referenceRequired/)
