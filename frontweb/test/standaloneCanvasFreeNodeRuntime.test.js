...
test('自由节点配置面板保存并回填模型、比例和时长字段，并记住视频参数', () => {
  ...
  assert.match(canvasSource, /function getFreeNodeModelOptions\(kind\) \{[\s\S]*\.map\(\(item\) => item\.model\)/)
  assert.match(canvasSource, /<el-option v-for="model in getFreeNodeModelOptions\(freeNodeKind\)" :key="model" :label="model" :value="model" \/>/)
