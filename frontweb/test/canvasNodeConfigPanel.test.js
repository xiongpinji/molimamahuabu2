...
test('分镜配置面板提供统一节点执行和失败恢复入口', () => {
  ...
  assert.match(nodeExecutionStripSource, /const canUseResultAsDownstreamReference = computed\(\(\) => Boolean\(resultUrl\.value\) && Boolean\(downstreamNode\.value\?\.id\) && Boolean\(ctx\?\.useNodeResultAsDownstreamReference\)\)/)
