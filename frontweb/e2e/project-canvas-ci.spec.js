...
test('项目画布节点编辑器跟随节点且不会撑大页面', async ({ page }) => {
  savedCanvasLayout = {
    version: 1,
    viewport: { x: 180, y: 250, zoom: 0.6 },
    nodes: {},
    manual_edges: [],
    free_nodes: [{
      id: 'e2e:editor-follow',
      type: 'homeCanvasNode',
      position: { x: 520, y: 620 },
      data: { kind: 'image', title: '编辑框跟随测试节点', content: '' },
    }],
  }
  mockDrama.metadata.project_type = 'canvas'
  await page.setViewportSize({ width: 900, height: 1000 })
  await page.goto('/canvas/3')

  const node = page.locator('.vue-flow__node[data-id="e2e:editor-follow"]')
  const visualNode = node.locator('.home-canvas-node')
  ...
  const assertEditorLayout = async () => {
    const viewport = page.viewportSize()
    const [nodeBox, editorBox, layout, documentLayout] = await Promise.all([
    ...
    const expectedLeft = Math.min(
      Math.max(16, nodeBox.x + nodeBox.width / 2 - editorBox.width / 2),
      Math.max(16, viewport.width - editorBox.width - 16),
    )
    expect(Math.abs(editorBox.x - expectedLeft)).toBeLessThan(5)
    ...
    return { nodeBox, editorBox }
  }

  await assertEditorLayout()
  const nodeIcon = node.locator('.node-icon')
  const dragBox = await nodeIcon.boundingBox()
  await page.mouse.move(dragBox.x + dragBox.width / 2, dragBox.y + dragBox.height / 2)
  await page.mouse.down()
  await page.mouse.move(dragBox.x + dragBox.width / 2 + 90, dragBox.y + dragBox.height / 2 - 60, { steps: 6 })
  await page.mouse.up()

  await expect.poll(async () => {
    const [nodeBox, editorBox] = await Promise.all([visualNode.boundingBox(), editor.boundingBox()])
    const viewport = page.viewportSize()
    const expectedLeft = Math.min(
      Math.max(16, nodeBox.x + nodeBox.width / 2 - editorBox.width / 2),
      Math.max(16, viewport.width - editorBox.width - 16),
    )
    return Math.abs(editorBox.x - expectedLeft)
  }).toBeLessThan(5)
  await assertEditorLayout()

  const canvas = page.locator('.canvas-main')
  const transformBefore = await transformationPane.evaluate((element) => element.style.transform)
  await canvas.hover({ position: { x: 760, y: 180 } })
  await page.keyboard.down('Control')
  await page.mouse.wheel(0, -120)
  await page.keyboard.up('Control')
  await expect.poll(() => transformationPane.evaluate((element) => element.style.transform)).not.toBe(transformBefore)
  await assertEditorLayout()

  const transformBeforeEdgeDrag = await transformationPane.evaluate((element) => element.style.transform)
  const edgeDragBox = await nodeIcon.boundingBox()
  await page.mouse.move(edgeDragBox.x + edgeDragBox.width / 2, edgeDragBox.y + edgeDragBox.height / 2)
  await page.mouse.down()
  await page.mouse.move(edgeDragBox.x + edgeDragBox.width / 2, edgeDragBox.y + edgeDragBox.height / 2 + 320, { steps: 8 })
  await page.mouse.up()

  await expect(editor).toHaveAttribute('data-editor-dock', 'viewport')
  const [edgeNodeBox, edgeEditorBox] = await Promise.all([
    visualNode.boundingBox(),
    editor.boundingBox(),
  ])
  const edgeViewport = page.viewportSize()
  const edgeDocumentLayout = await page.evaluate(() => ({
    bodyClientHeight: document.body.clientHeight,
    bodyClientWidth: document.body.clientWidth,
    bodyScrollHeight: document.body.scrollHeight,
    bodyScrollWidth: document.body.scrollWidth,
  ...
  expect(edgeNodeBox).not.toBeNull()
  expect(edgeEditorBox).not.toBeNull()
  expect(edgeEditorBox.width).toBeGreaterThanOrEqual(258)
  expect(edgeEditorBox.x).toBeGreaterThanOrEqual(0)
  expect(edgeEditorBox.y).toBeGreaterThanOrEqual(0)
  expect(edgeEditorBox.x + edgeEditorBox.width).toBeLessThanOrEqual(edgeViewport.width)
  expect(edgeEditorBox.y + edgeEditorBox.height).toBeLessThanOrEqual(edgeViewport.height)
  const edgeOverlapWidth = Math.max(0, Math.min(edgeEditorBox.x + edgeEditorBox.width, edgeNodeBox.x + edgeNodeBox.width) - Math.max(edgeEditorBox.x, edgeNodeBox.x))
  const edgeOverlapHeight = Math.max(0, Math.min(edgeEditorBox.y + edgeEditorBox.height, edgeNodeBox.y + edgeNodeBox.height) - Math.max(edgeEditorBox.y, edgeNodeBox.y))
  expect(edgeOverlapWidth * edgeOverlapHeight).toBeLessThan(1)
  expect(await transformationPane.evaluate((element) => element.style.transform)).toBe(transformBeforeEdgeDrag)
  expect(edgeDocumentLayout.bodyScrollHeight).toBeLessThanOrEqual(edgeDocumentLayout.bodyClientHeight + 1)
  expect(edgeDocumentLayout.bodyScrollWidth).toBeLessThanOrEqual(edgeDocumentLayout.bodyClientWidth + 1)
  expect(edgeDocumentLayout.rootScrollHeight).toBeLessThanOrEqual(edgeDocumentLayout.rootClientHeight + 1)
  expect(edgeDocumentLayout.rootScrollWidth).toBeLessThanOrEqual(edgeDocumentLayout.rootClientWidth + 1)
})
