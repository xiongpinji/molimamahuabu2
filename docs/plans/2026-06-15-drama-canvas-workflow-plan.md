...
- [x] 全能模式画布内润色 / 流式编辑全能词
- [x] 工作流组内分镜顺序可视化拖拽调整
- [x] 多集同时展示时的节点虚拟化（仅渲染视口附近，并保留已选/聚焦节点）
- [x] 画布内首尾帧单独生图（走 frame-prompt 模块，对齐列表模式完整能力）
- [x] 顶栏工作流区域增加简短帮助 tooltip
- [x] 画布生成参数面板（模型、画幅、视频清晰度）与单镜/批量请求统一
- [x] 当前集 / 全部集切换统一约束画布素材节点、素材库与关系高亮上下文
...
- [x] 分镜参考图自由上传（列表模式已有部分能力，画布侧统一入口）
- [x] 参考图自由选择（生成分镜图时手动指定角色/场景参考）
