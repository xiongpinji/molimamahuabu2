...
适配器在提交前对上述数量和时长做硬校验，超限直接返回错误，不静默截断、不预扣积分、不调用供应商。fumin 文档明确给出 `content` 多模态数组及 `text`、`image_url` 结构，本适配器按同一内容数组约定发送带 `role` 的 `image_url`、`video_url`、`audio_url` 参考项。

真实验证证据（2026-08-14，本地临时文件，不含 Key）：FAST 任务 `task_sOjTcSpxci9NClU9EmVQdFynJTn7VRJW` 和 MINI 任务 `task_NL7rAyQ9VXC2TjwxFXPRJXszAzlhI2OV` 均使用 15 秒、480P、16:9、9 张参考图、3 个视频参考、3 个音频参考完成；结果 MP4 均可下载并由 ffprobe 读取，时长均为 15.104 秒。供应商实际校验还确认视频参考总时长和音频参考总时长各自不得超过 15.2 秒，测试素材按该边界准备。上述证据覆盖本次开放的声明能力；后续若供应商策略变化，仍须保留失败终态并按真实结果收窄能力。
