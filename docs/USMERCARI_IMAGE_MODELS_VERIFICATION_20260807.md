...
开放门禁未通过，禁止把本次两模型写入前端供应商预设、用户模型目录或生产 `ai_service_configs`。
...
按照项目外部模型硬门禁，“7 个成功 + 1 个失败”不能视为整组通过。后续计价、管理员配置、首页、画布和短剧工厂开放任务已停止，等待供应商修复 GPT 4K，或由产品明确缩小为经过验证的档位集合后重新制定开放门禁。
