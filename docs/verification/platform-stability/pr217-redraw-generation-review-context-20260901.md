...
### 2026-09-04 r22 真实门禁与对白保留裁剪修复

- PR #217 `HEAD=2cf002abe334005ace1bce162e95c9a30a38bbf8` 的 Hosted CI 四项全部通过后，按用户新授权创建全新隔离状态 `pr217-fumin-full-episode-20260904-r22-delivery`。preflight 为 28 个执行单元、0 个任务，包 SHA-256 `eb3777440e662d4b0831a3b6fc0bf342a367eef654d8dff2da6c77fc75321091`，执行计划哈希 `02c3d2062eba25c92b4241d912ff663aace22ff2d0274080d7edc07daaea77d3`。
- r22 只对 `shot-24.part-01` 上传 1 张 Ethan Cole 身份图和 1 段动作参考，并创建唯一供应商任务 `task_wbc8MEp3s19LAScWJaKC7vO183TRbGUO`。原始结果 SHA-256 `4c83c246477fd18ce2a8028542d798738e5df4a4046acc60dc360c179381a146`，媒体为 H.264、496x864、24 fps、5.088 秒并含 AAC 双声道音轨。
- 原始结果本身通过严格英文对白门禁：base 与 small ASR 均逐字识别 `My fortune starts with the World Cup.`，语言概率分别约 0.991 与 0.994。失败发生在本地规范化之后：旧实现固定从 0 秒截取到本单元 3.766 秒，供应商语音实际约位于 2.06–4.26 秒，截尾后两套 ASR 分别只剩 `My fortune starts with the` 与 `My fortune starts with a`，任务因此以 `FUMIN_EPISODE_ASR_CONSENSUS_FAILED` 停止。
- 失败后严格保持不重试，后续 27 个单元均为 0 提交。根因不在模型、Key、角色或英文提示，而在本地规范化忽略已验收语音时间范围。
- TDD 红灯先稳定得到 34 项中 32 通过、2 失败，证明媒体参数始终固定 `-ss 0.000` 且适配器未传递语音窗口；最小修复让双 ASR 保留实际语音起止时间，并在不改变 5 秒供应商请求、目标时长、480p 或模型的前提下移动本地截取窗口以完整包住对白。绿灯为 34/34 通过。
- 使用 r22 已下载原片执行零网络离线重放，供应商 GET/POST/上传/生成/计费均为 0。修复后 3.766 秒规范化文件 SHA-256 `16017d8c6b0b1da79cdbc653b5d158181edb754d7c976156773ac71adee6b20b`；base 与 small ASR 再次逐字一致识别批准对白，`en-US`、480x864、H.264、AAC、双声道和目标时长均通过。
- 本次修改已登记到 `redraw.episode-blueprint-first` 功能锁，旧的结果地址兼容批准完整保留于 `unlockHistory`；新的对白保留裁剪批准绑定媒体管线、供应商适配器、两个整集入口和功能锁测试。
- 扩大后的 Fumin 与整集执行本地回归共 133 项，131 通过、2 项因当前 Windows 主机无符号链接权限跳过、0 失败；前端生产构建完成 1921 个模块并退出 0；功能锁与增量发布范围回归 66/66 通过。
- r22 继续作为失败且不可重试的历史状态保留。任何新供应商提交必须绑定修复后的新 HEAD、使用全新隔离状态并取得新的明确授权；不得续跑或改写 r22。
