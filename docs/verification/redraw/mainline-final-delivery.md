...
#### G3 多输入与执行失败矩阵收口（2026-09-10）

当前基线 HEAD 为 `77412a194c2e65ee40251968e52133555bcc1f17`，以下为该基线上尚未提交的七个测试/夹具/计划/报告文件差异，不借用旧 HEAD 的 CI 结论。默认真实 HTTP ZIP 产品链新增第三份本轮 FFmpeg 媒体：横屏 192×128／12 秒／无音轨，竖屏 128×192／14 秒／AAC＋对白，以及方形 160×160／12 秒／AAC 背景音。三份 source SHA 各异，均经普通登录、默认租户、multipart 上传、真实资产保存、FFprobe、默认分析与 owner 隔离；最低层 ASR/视觉客户端为确定性替身，其中音乐输入的 Worker 替身在真实抽取 WAV 上返回 VAD 零语音。音乐输入因此保持 AAC 事实、空 dialogue、`dialogue_mode=silent`、`source_language=null`；这证明产品分支和媒体绑定，不证明真实 ASR/VAD 内容质量。全程没有生成、TTS、发布或生产写入。

当前动态测试终态：多输入＋窗口计划＋执行计划组合 **51/51**；`redrawExecutionAdvance.test.js` 修复后完整 **25/25**（541,178 ms）；前端 run/candidate/panel 生命周期 **216/216**；后端 unit review/recovery **74/74**。各命令均 native exit 0、0 fail/skip/cancel。第二单元失败使用真实隔离 SQLite、真实候选媒体及三个父镜头的真实动作派生/准备队列：首单元成功且人工批准后才提交第二单元；第二单元明确失败使 run 停止，固定 2 POST／1 下载／2 reservation，失败 reservation 退款一次，第三单元零 attempt，重复 advance 零 POST/零 DML。恢复套件覆盖已知 ID 显式恢复、无 ID unknown 冻结、刷新/服务重挂、迟到结果、候选读取、批准/拒绝、owner/tenant ABA；前端套件覆盖同作用域恢复和防重复操作。

提交前冻结的五个可执行测试／fixture 源 SHA-256：`redrawExecutionUnitDispatchFixture.js`=`7cabfa9cc14dd8b909708f4696d19320a22d793289f155d29bac611b96855a7b`；`redrawMultiInputProductAnalysisHttpFixture.js`=`79eda893b16c6491a8e05b0a8f32b8b7463ecef68fa7a95d9b4fc01dd91bb322`；`redrawMultiInputProductAnalysisHttpGuard.js`=`c47c02b778737aa6e730940a13df0a1a7672b13ca5b728f65d9e1d0f2fe61542`；`redrawExecutionAdvance.test.js`=`9709519bc2dba9258cf1d0800e3e1ef5f8d9cbf18df574fb24f7ab4b0aadfaac`；`redrawMultiInputProductAnalysisHttp.test.js`=`0f54dc4374e65088fd98399f0146bf6a8236b30b38826acdd64df54d7fe01078`。计划和本报告因记录该清单而自引用变化，不用预写 hash 冒充最终提交值；最终 40 位提交 HEAD 与 Git blob 由获授权后的提交阶段记录。

本批只补覆盖和收口报告，没有改变产品运行时代码、模型、供应商配置或生产数据。精确跟踪差异为七个文件：五个测试／fixture 文件、唯一计划和本总报告；`.codex-staging/`、`output/`、`docs/verification/redraw/recorder-diagnostics-20260905/` 是既有未跟踪内容，未删除、未纳入本批。仍未完成且不得冒称：本批未提交/推送，最终 40 位新 HEAD 尚不存在，因此同 HEAD Hosted CI 未运行；没有真实供应商内容质量、真实 ASR 长语音质量、正式视觉基线、任意新用户视频或用户接受证据，也没有部署。通用一键转绘只能在精确提交、Hosted CI 四项全绿、独立真实输入质量与用户页面验收分别完成后宣称完整交付。
