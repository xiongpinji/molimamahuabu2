...
发布审计逐项绑定用例 ID、模型、模式、请求分辨率、请求时长、角色数组、同步音频布尔值、ffprobe 实际尺寸/时长、唯一供应商任务、唯一本站成品、SHA-256、余额前后差额、人民币换算、费用复核状态和真实验证速度。速度证据包含提交 POST 往返取得 `task_id` 的 `submit_latency_ms`、从提交开始到完成的 `generation_elapsed_seconds`、`started_at`、`completed_at`，并在 evidence 中输出按 model/resolution/mode 的明细及 fast/mini 的样本数、min/max/avg 汇总；该汇总仅表示本轮实测，不是供应商 SLA。恢复或费用确认的零 POST 重跑只复验原任务和文件，不改写原始速度。复制同一任务或成品、伪造 720P 尺寸、缺少参考角色、篡改请求参数、账单差额或速度证据都不能升级配置为 `verified`。
