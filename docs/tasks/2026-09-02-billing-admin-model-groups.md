...
#### 2026-09-03 NewAPI 配置级能力与路由隔离

- 用户明确指出本任务与 ToAPIs 无关；已撤回 ToAPIs 付费复验路径。本轮不请求该授权，也不调用任何供应商生成接口。
- 根因是 NewAPI 的 4 秒例外和限定计费身份曾混在共享视频服务的通用模型判断中，同名模型可能跨中转站继承能力；共享门禁又因此把 NewAPI 变化误判为 ToAPIs 私人形象变化。
- 修复后六个 NewAPI 模型均绑定 `cfg-29::模型ID`，4 秒/5 秒、分辨率、画幅、参考和音频限制只来自配置 29 的真实验证能力。任务创建固化配置版本，配置变化时拒绝供应商提交；其他中转站同名模型不继承 NewAPI 能力。
- NewAPI 视频限定还要求 `service_type=video`；新增反例证明 NewAPI 图片配置继续保留无前缀模型身份，不会被视频规则污染。
- 管理员 NewAPI 预设同步为六个精确 ID，并只展示真实生成验证组合；首页与短剧工厂测试数据也使用六个配置限定 ID，避免前端测试掩盖无前缀路由回退。
- 发布门禁只忽略精确列举的 NewAPI 专属语句，并新增反例证明扩大配置固定范围仍会被检测；ToAPIs 私人形象投影与改动前基线逐字一致。
- 本地复核为后端主矩阵 `332/335` 通过、0 失败、3 项 Windows 跳过，故障恢复与供应商路由 `169/169`，前端全量 `975/975`，功能锁 `ready=true`，生产构建成功。
- 该修复不单独部署，由当前脚本分析生产修复会话在同一受保护候选中叠加，防止两个会话互相覆盖。
