...
2. 已使用目标 Key 完成真实生成并确认成功终态、结果文件可读取；该记录证明目标 Key、模型和线路确实能够生成可读视频，不冒充逐参数付费覆盖证据。
3. 2026-08-31 产品负责人明确批准按供应商合同开放完整参数范围：`480P/720P/1080P`、2–30 秒、有声或静音、首尾帧、最多 10 图/5 视频/5 音频参考；能力来源在管理端和任务证据中标明为供应商合同，真实 smoke 证据范围仍如实保留为 `480P / 2 秒 / 16:9 / 静音 / 纯文本`。
4. 完整能力使用现有独立 Wan3 配置原位升级，保留配置 ID、Key、真实生成 evidence 和凭据绑定；三档分辨率保持相同的用户积分、模型成本与线路成本，不复制新模型、不覆盖 FAST/MINI。
5. 生产模型目录仍须经过本地回归、PR、Hosted CI、实时 `current` 构建的受保护增量候选、共享 verify-only 和单独授权激活；本次升级不再追加供应商调用或付费测试。
...
- 本轮只证明 `480P / 2 秒 / 16:9 / 静音 / 纯文本` 的真实生成与产物可读性；完整能力范围的开放依据另见 2026-08-31 产品合同批准记录，不把本轮 smoke 证据扩写成逐参数实测。
...
- 产品负责人批准直接按已核对的供应商 Wan 3.0 合同开放完整能力，不再追加付费实测：`480P/720P/1080P`、2–30 秒整数、`adaptive/16:9/9:16/1:1/4:3/3:4`、音频开关、首尾帧，以及最多 10 张图片、5 个视频、5 个音频参考。
- 真实生成记录继续只承担“目标 Key + `wan3.0-video` + ToAPIs 线路能够成功生成可读视频”的可用性证明；公开能力范围单独标记为经产品批准的供应商合同，二者不得混淆。
- 生产升级必须在同一事务中把现有 Wan3 配置原位升级：配置 ID、API Key、凭据指纹、真实生成 evidence、模型成本和线路成本逐值保持；只扩展名称、公开能力标识、三档分辨率价格镜像和完整运行时能力。
- 用户积分统一为每秒 134 积分，`480P/720P/1080P` 三档相同；模型成本和线路成本沿用现有 Wan3 配置值，三档相同。发现既有配置、证据、价格或线路成本漂移时失败关闭，不新增配置、不覆盖漂移数据。
- 本轮禁止新增供应商 GET/POST、上传、生成或付费；只有代码回归、Hosted CI、受保护候选 verify-only 和激活后零付费检查全部通过，才可声明线上升级完成。
