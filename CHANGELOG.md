...
- **LibTV 风格画布工作区**：首页自由画布与项目画布统一工作区交互，支持右键添加节点、复制/插入/追加、快捷键、`Ctrl + 滚轮` 缩放、空格拖动画布、工作流分组和节点运行结果闭环
- **真实生成与素材复用**：图片、视频、首尾帧、音频和脚本节点接通真实任务状态、失败写回、重试恢复、结果入库、下游引用及素材回填
- **视频与音色能力扩展**：接入 DeepWL Grok、iCreat Seedance 与 AIHubCC 图片/视频协议；支持分镜单镜模型、角色声线固化、音色素材库与不支持克隆模型的提示词补偿
- **3D 导演台生产闭环**：支持真实角色/场景资源、镜头实体与序列、切点/转场、角色轨道、动作片段、保存恢复和视频导出状态
- **公开平台管理能力**：增加多租户隔离、管理员账号、兑换码、积分流水、模型独立计费、对账、订阅基础与租户控制台
- **生产运维能力**：增加预发布预检、数据库备份/校验/恢复演练、依赖安全持续门禁和 Windows 桌面构建流水线

### 优化

- **画布节点配置**：统一文本、图片、视频、音频、角色和脚本节点的模型选择、素材挂载、提示词编辑、执行入口、运行审计与结果预览
- **桌面发行**：统一茉莉妈妈品牌、安装身份和用户数据目录，增加覆盖安装、卸载保留、Authenticode 签名、时间戳、校验和、产物证明与草稿 Release 门禁
- **依赖安全**：升级后端高危图片/压缩依赖和 Electron 构建链，并在 CI 中持续阻止生产依赖高危回归

### 修复

- 修复供应商任务失败与余额不足时的误重试、图片失败状态残留、首尾帧衔接失败恢复、素材库与音色库旧接口降级等问题
- 修复 VueFlow 选择键警告、画布视口缩放状态、节点结果定位、素材路径兼容和分镜角色声线未随视频请求传递等问题

### 发布

- `frontweb`、`backend-node`、`desktop` 及其 lock 文件根版本统一为 **1.2.9**
- 正式 `v1.2.9` 标签仍要求仓库配置真实 Windows 签名证书；签名、时间戳、安装回归、校验和与产物证明全部通过后只创建草稿 Release
