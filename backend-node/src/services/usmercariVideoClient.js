...
const USMERCARI_MODELS = Object.freeze({
  'MiniMax H3': Object.freeze({ ...SHARED_CAPABILITIES, resolutions: Object.freeze(['480p']) }),
  'seedance-2.0-fast': Object.freeze({ ...SHARED_CAPABILITIES, resolutions: Object.freeze(['480p', '720p']) }),
  'seedance-2.0-mini': Object.freeze({ ...SHARED_CAPABILITIES, resolutions: Object.freeze(['480p', '720p']) }),
});
...
function validateUsmercariVideoOptions(opts = {}) {
  ...
  if (duration !== 5) throw new Error('USMercari 三个视频模型目前仅开放已实测的 5 秒时长');
  ...
  const imageCount = Math.max(images.length, uniqueValues(Array.isArray(opts.image_ids) ? opts.image_ids : []).length);
  const videoCount = Math.max(videos.length, opts.video_reference_id ? 1 : 0);
  const audioCount = Math.max(audio.length, opts.audio_reference_id ? 1 : 0);
  if (imageCount > spec.maxImages) throw new Error(`USMercari 模型 ${model} 最多支持 ${spec.maxImages} 张参考图`);
  if (videoCount > spec.maxVideos) throw new Error(`USMercari 模型 ${model} 最多支持 ${spec.maxVideos} 个参考视频`);
  if (audioCount > spec.maxAudio) throw new Error(`USMercari 模型 ${model} 最多支持 ${spec.maxAudio} 个参考音频`);
  if ((hasFirstFrame || hasLastFrame) && imageCount) throw new Error('USMercari 首尾帧模式与多图参考模式互斥');
  if (hasLastFrame && !hasFirstFrame) throw new Error('USMercari 尾帧必须与首帧一起使用');
  if (videoCount && !hasFirstFrame) throw new Error('USMercari 参考视频模式必须同时提供首帧');
  if (audioCount && !hasFirstFrame && !imageCount) throw new Error('USMercari 参考音频必须同时提供首帧或参考图');
...
function buildUsmercariVideoBody(opts = {}) {
  ...
  if (Array.isArray(opts.image_ids) && opts.image_ids.length) metadata.image_ids = uniqueValues(opts.image_ids).slice(0, checked.spec.maxImages);
  if (opts.video_reference_id) metadata.video_reference_id = String(opts.video_reference_id);
  if (opts.audio_reference_id) metadata.audio_reference_id = String(opts.audio_reference_id);
  return {
    model: checked.model,
...
async function mediaUploadPayload(kind, source, opts = {}) {
  ...
  if (dataMatch) {
    ...
    assertMediaSize(kind, bytes.length);
    return {
      data: raw,
      extension: extensionFromMime(dataMatch[1], DEFAULT_EXTENSION[kind]),
    };
  }
  ...
  if (localFile) {
    ...
    return { data: `data:${mimeForExtension(extension, kind)};base64,${bytes.toString('base64')}`, extension };
  }
  ...
  if (contentLength) assertMediaSize(kind, contentLength);
  ...
  return { data: `data:${mime};base64,${bytes.toString('base64')}`, extension };
}
...
async function uploadUsmercariMedia(config, kind, source, opts = {}) {
  ...
  const response = await fetch(`${normalizeUsmercariBaseUrl(config?.base_url)}/v1/media/upload/${kind}`, {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(`USMercari 上传参考${kind}失败 (${response.status}): ${String(message).slice(0, 300)}`);
  }
  const id = payload?.id ?? payload?.data?.id;
  if (id == null || String(id).trim() === '') throw new Error(`USMercari 上传参考${kind}成功但未返回 media id`);
  return String(id);
}
...
async function prepareUsmercariVideoBody(config, opts = {}) {
  ...
  const [imageId, endImageId, imageIds, videoReferenceId, audioReferenceId] = await Promise.all([
    ...
    checked.videos[0] ? uploadUsmercariMedia(config, 'video', checked.videos[0], uploadOpts) : '',
    checked.audio[0] ? uploadUsmercariMedia(config, 'audio', checked.audio[0], uploadOpts) : '',
  ]);
...
function parseUsmercariFetchPayload(payload, taskId, baseUrl = 'https://ai.usmercari.com') {
  ...
  const task = tasks.find((item) => String(item?.task_id || '') === String(taskId)) || tasks[0];
  ...
  const result = items.flatMap((item) => Array.isArray(item?.data) ? item.data : [])
