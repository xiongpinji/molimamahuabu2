...
function getReservation(db, id) {
  return db.prepare('SELECT * FROM usage_reservations WHERE id = ?').get(String(id)) || null;
}

function validateReservationInput(input) {
  const amount = Number(input.amount);
  if (!input.userId || !input.operationKey || !input.model || !input.resourceType || input.resourceId == null) {
...
module.exports = { ensureSchema, setAccountBalance, getAccount, getReservation, reserve, confirm, refund, settleGeneration };
