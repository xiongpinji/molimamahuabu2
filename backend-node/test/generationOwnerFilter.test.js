...
function setup() {
  ...
  const image = db.prepare(`
    INSERT INTO image_generations (drama_id, model, status, user_id, created_at, updated_at)
    VALUES (?, ?, 'completed', ?, ?, ?)
  `).run(1, 'gpt-image-2', 'user-1', now, now);
  const imageOther = db.prepare(`
    INSERT INTO image_generations (drama_id, model, status, user_id, created_at, updated_at)
    VALUES (?, ?, 'completed', ?, ?, ?)
  `).run(1, 'gpt-image-2', 'user-2', now, now);
  const video = db.prepare(`
    INSERT INTO video_generations (drama_id, model, status, user_id, created_at, updated_at)
    VALUES (?, ?, 'completed', ?, ?, ?)
  `).run(1, 'seedance 2.0', 'user-1', now, now);
  const videoOther = db.prepare(`
    INSERT INTO video_generations (drama_id, model, status, user_id, created_at, updated_at)
    VALUES (?, ?, 'completed', ?, ?, ?)
  `).run(1, 'seedance 2.0', 'user-2', now, now);
