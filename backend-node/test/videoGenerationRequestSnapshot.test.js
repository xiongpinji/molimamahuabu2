...
function insertCompletedImageGeneration(db, dramaId, relPath) {
  const now = new Date().toISOString();
  return Number(db.prepare(
    `INSERT INTO image_generations
      (drama_id, provider, prompt, model, image_url, local_path, status, completed_at, created_at, updated_at)
     VALUES (?, 'test-image-provider', 'AI 生成人物', 'test-image-model', ?, ?, 'completed', ?, ?, ?)`
  ).run(
    dramaId,
    `/static/${relPath}`,
    relPath,
...
function insertLingjingConfig(db, now, overrides = {}) {
  ...
  db.prepare(
    `INSERT INTO ai_service_configs
      (service_type, provider, api_protocol, name, base_url, api_key, model, default_model,
       is_active, is_default, priority, verification_status, verified_capabilities, created_at, updated_at)
     VALUES ('video', 'lingjing', 'lingjing_open', '灵境测试', 'https://seed.alimyun.xyz/api/open/v1',
       'test-key', ?, 'lingjing-video-v1', 1, 0, 0, ?, ?, ?, ?)`
  ).run(
    JSON.stringify(['lingjing-video-v1']),
    overrides.verificationStatus || 'verified',
    JSON.stringify({ 'lingjing-video-v1': capabilities }),
    now,
    now,
  );
  prices.set(db, 'lingjing-video-v1', 69, {
    category: 'video',
    billing_unit: 'second',
    cost_unit: 'second',
...
test('Lingjing create stores only verified image references and omits supplier resolution', (t) => {
  const { db, drama1, now } = setup(t);
  insertLingjingConfig(db, now);
  insertAsset(db, drama1, 'image', 'projects/0001/assets/lingjing-ref.png');
  ...
  const created = createVideo(db, {
    ...
    aspect_ratio: '16:9',
    resolution: '720p',
    reference_image_urls: ['/static/projects/0001/assets/lingjing-ref.png'],
  ...
  assert.equal(row.model, 'lingjing-video-v1');
  ...
  assert.deepEqual(snapshot.reference_image_urls, ['https://molimama.vip/static/projects/0001/assets/lingjing-ref.png']);
});

test('Lingjing rejects unsupported modes and stale evidence before any side effect', (t) => {
  for (const [body, expectedCode] of [
    [{ first_frame_url: 'https://example.com/first.png' }, 'VIDEO_REFERENCE_FORBIDDEN'],
    [{ reference_video_urls: ['https://example.com/ref.mp4'] }, 'UNSUPPORTED_VIDEO_REFERENCE'],
    [{ reference_audio_urls: ['https://example.com/ref.mp3'] }, 'UNSUPPORTED_VIDEO_REFERENCE'],
  ]) {
    const { db, drama1, now } = setup(t);
    insertLingjingConfig(db, now);
  ...
  const { db, drama1, now } = setup(t);
  insertLingjingConfig(db, now, { verificationStatus: 'pending' });
  assert.throws(() => createVideo(db, {
    drama_id: drama1,
    storyboard_id: 1,
    model: 'lingjing-video-v1',
...
test('Lingjing processing rechecks the pinned config and submits the immutable snapshot', async (t) => {
  const { db, drama1, now } = setup(t);
  insertLingjingConfig(db, now);
  insertAsset(db, drama1, 'image', 'projects/0001/assets/lingjing-process.png');
  const scheduled = [];
  const submitted = [];
  const originalCall = videoClient.callVideoApi;
  videoClient.callVideoApi = async (_db, _log, request) => {
    submitted.push(request);
    return { error: 'controlled supplier stop' };
  };
  ...
  const created = createVideo(db, {
    ...
    aspect_ratio: '16:9',
    resolution: '720p',
    reference_image_urls: ['/static/projects/0001/assets/lingjing-process.png'],
  }, {
    billingEnabled: true,
    tenantId: 'tenant-1',
    userId: 'user-1',
  ...
  const stored = db.prepare('SELECT ai_service_config_id, source_conditioning_json FROM video_generations WHERE id = ?')
    .get(created.id);
  assert.ok(Number(stored.ai_service_config_id) > 0);
  assert.equal(JSON.parse(stored.source_conditioning_json).video_capability.model, 'lingjing-video-v1');

  await scheduled[0]();
  assert.equal(submitted.length, 1);
  assert.equal(submitted[0].model, 'lingjing-video-v1');
  assert.equal(submitted[0].duration, 4);
  assert.equal(submitted[0].resolution, null);
  assert.deepEqual(submitted[0].reference_urls, ['https://molimama.vip/static/projects/0001/assets/lingjing-process.png']);
  assert.equal(submitted[0].first_frame_url, null);
...
test('ToAPIs snapshot binds only server-proven AI image assets for private-avatar conversion', async (t) => {
  const { db, drama1 } = setup(t);
  const relPath = 'projects/0001/assets/ai-character.png';
  const imageGenerationId = insertCompletedImageGeneration(db, drama1, relPath);
  const assetId = insertAsset(db, drama1, 'image', relPath);
  db.prepare('UPDATE assets SET image_gen_id = ? WHERE id = ?').run(imageGenerationId, assetId);
  let submitted = null;
  const originalCall = videoClient.callVideoApi;
  ...
  const created = createVideo(db, {
    ...
    storyboard_id: 1,
    model: 'seedance-2-mini',
    prompt: 'AI 人物参考图自动转虚拟人像',
    duration: 4,
    reference_image_urls: [`/static/${relPath}`],
    _toapis_private_avatar_images: [{
  ...
  const snapshot = JSON.parse(db.prepare(
    'SELECT request_snapshot FROM video_generations WHERE id = ?'
  ).get(created.id).request_snapshot);
  assert.deepEqual(snapshot._toapis_private_avatar_images, [{
    url: 'https://molimama.vip/static/projects/0001/assets/ai-character.png',
    source_kind: 'asset',
    source_id: assetId,
  }]);

  await videoService.processVideoGeneration(db, log, created.id);
  assert.deepEqual(submitted._toapis_private_avatar_images, snapshot._toapis_private_avatar_images);
});

test('ToAPIs private-avatar binding prefers the proven generation asset over a newer canvas copy', (t) => {
  const { db, drama1 } = setup(t);
  const relPath = 'projects/0001/assets/ai-character-copy.png';
  const imageGenerationId = insertCompletedImageGeneration(db, drama1, relPath);
  const generationAssetId = insertAsset(db, drama1, 'image', relPath);
  db.prepare('UPDATE assets SET image_gen_id = ? WHERE id = ?').run(imageGenerationId, generationAssetId);
  insertAsset(db, drama1, 'image', relPath);

  const created = createVideo(db, {
    drama_id: drama1,
    storyboard_id: 1,
    model: 'seedance-2-mini',
    prompt: '画布副本不能遮蔽 AI 生成来源',
    duration: 4,
    reference_image_urls: [`/static/${relPath}`],
  }, { billingEnabled: false, schedule() {} });
  ...
  assert.deepEqual(snapshot._toapis_private_avatar_images, [{
    url: 'https://molimama.vip/static/projects/0001/assets/ai-character-copy.png',
    source_kind: 'asset',
    source_id: generationAssetId,
  }]);
});

test('ToAPIs private-avatar binding falls back to the proven image generation behind a canvas copy', (t) => {
  const { db, drama1 } = setup(t);
  const relPath = 'projects/0001/assets/ai-character-generation.png';
  const imageGenerationId = insertCompletedImageGeneration(db, drama1, relPath);
  insertAsset(db, drama1, 'image', relPath);
  ...
  const created = createVideo(db, {
    ...
    storyboard_id: 1,
    model: 'seedance-2-mini',
    prompt: '画布副本不能遮蔽 image_generations 来源',
    duration: 4,
    reference_image_urls: [`/static/${relPath}`],
  }, { billingEnabled: false, schedule() {} });
  ...
  assert.deepEqual(snapshot._toapis_private_avatar_images, [{
    url: 'https://molimama.vip/static/projects/0001/assets/ai-character-generation.png',
    source_kind: 'image_generation',
    source_id: imageGenerationId,
  }]);
});

test('ToAPIs snapshot never marks ordinary uploaded images as private-avatar sources', (t) => {
  const { db, drama1 } = setup(t);
  const unrelatedGenerationId = insertCompletedImageGeneration(
    db,
    drama1,
    'projects/0001/assets/other-ai-image.png'
  );
  const assetId = insertAsset(db, drama1, 'image', 'projects/0001/assets/uploaded-person.png');
  db.prepare('UPDATE assets SET image_gen_id = ? WHERE id = ?').run(unrelatedGenerationId, assetId);

  const created = createVideo(db, {
    drama_id: drama1,
    storyboard_id: 1,
    model: 'seedance-2-mini',
    prompt: '普通上传图不得自动转虚拟人像',
    duration: 4,
    reference_image_urls: ['/static/projects/0001/assets/uploaded-person.png'],
  }, { billingEnabled: false, schedule() {} });
...
test('ToAPIs process uses request_snapshot arrays and does not rebuild from mutated row fields', async (t) => {
  ...
  assert.equal(captured.reference_mode, 'omni');
