...
const { callAihubccImageApi, callImageApi } = require('../src/services/imageClient');
...
test('callImageApi routes an AIHubCC base URL through the flow adapter even when config metadata says openai', async () => {
  const originalFetch = global.fetch;
  const calls = [];
  global.fetch = async (url, options = {}) => {
    calls.push({ url, options });
    return jsonResponse({
      choices: [{ message: { content: '![image](https://cdn.example.com/aihubcc-flow.png)' } }],
    });
  };
  const row = {
    id: 17,
    ...
    provider: 'openai',
    api_protocol: 'openai',
    name: 'AIHubCC GPT Image',
    base_url: 'https://aihubcc.cc/v1',
    api_key: 'test-key',
    model: JSON.stringify(['gpt-image-2-2k']),
  ...
  const db = {
    prepare(sql) {
      return {
        all: (...params) => sql.includes('ORDER BY is_default') && params.length === 1 ? [row] : [],
        run: () => ({ changes: 0 }),
      };
    },
  };
  try {
    const result = await callImageApi(db, { info() {}, warn() {}, error() {} }, {
      prompt: 'a cinematic portrait',
      model: 'gpt-image-2-2k',
    });
    assert.deepEqual(result, { image_url: 'https://cdn.example.com/aihubcc-flow.png' });
    assert.equal(calls.length, 1);
    assert.match(calls[0].url, /aihubcc\.cc\/v1\/chat\/completions$/);
    assert.match(calls[0].options.body, /cinematic portrait/);
  } finally {
    global.fetch = originalFetch;
  }
});
