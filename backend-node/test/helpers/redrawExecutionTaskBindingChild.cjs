...
// Local task-binding concurrency worker only. The caller supplies a newly created fixture database
  ...
  if (key === 'transaction') return callback => {
    ...
    wrapped.immediate = (...args) => {
      if (!announced) {
        ...
        emit({ type: 'binding_transaction_waiting', contention, probes,
          initial_tasks: target.prepare("SELECT count(*) n FROM async_tasks WHERE type='redraw_execution_unit'").get().n });
      }
...
const ctx = { db: guardedDb, tenantId: 'tenant-a', userId: 'user-a', storageRoot: payload.storageRoot,
  tempRoot: process.cwd(), env: {}, log: { info() {}, warn() {}, error() {} }, execFile: (binary, args, options, callback) => {
    assert.equal(binary, process.env.FFPROBE_PATH, 'binding inspection cannot encode'); probes += 1;
...
service.bindClaimedExecutionUnitTask(ctx, payload.versionId, payload.runId, payload.input).then(result => {
