...
test('注册新用户并创建零余额积分账户', () => {
  ...
  assert.equal(db.prepare("SELECT event_type FROM audit_events WHERE user_id = ?").get(result.body.data.user.id).event_type, 'auth.register.success');
});
