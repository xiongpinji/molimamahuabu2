...
test('ToAPIs pollVideoTask delegates GET polling without raw body or bearer log leakage', async () => {
  ...
  assert.equal(calls.every((call) => call.url === 'https://toapis.cn/v1/videos/generations/tsk_poll'), true);
...
test('shared video polling reads the ToAPIs task endpoint', async () => {
  ...
  assert.equal(requestedUrl, 'https://toapis.cn/v1/videos/generations/task-fast-1');
