...
test('ToAPIs callVideoApi converts a server-bound AI image into an active private-avatar asset', async (t) => {
  const originalSecret = process.env.PLATFORM_JWT_SECRET;
  process.env.PLATFORM_JWT_SECRET = 'test-provider-avatar-source-secret-at-least-32-characters';
  t.after(() => {
    if (originalSecret == null) delete process.env.PLATFORM_JWT_SECRET;
    else process.env.PLATFORM_JWT_SECRET = originalSecret;
  });
  const db = makeDb(makeVideoConfig());
  const sourceUrl = 'https://molimama.vip/static/projects/0039/ai-character.png';
  const avatarCalls = [];
  const generationCalls = [];

  const result = await callVideoApi(db, makeLog(), {
    model: 'seedance-2-mini',
    prompt: '让 AI 生成人物自然行走',
    duration: 4,
    resolution: '480p',
    aspect_ratio: '16:9',
    drama_id: 39,
    ...
    fetchImpl: async (url, init = {}) => {
      generationCalls.push({ url: String(url), body: init.body ? JSON.parse(init.body) : null });
      return {
        ok: true,
        status: 200,
        async text() { return JSON.stringify({ task_id: 'tsk_private_avatar', status: 'queued' }); },
      };
    },
  }, {
    evidenceRoots,
    ensureToapisPrivateAvatarAsset: async (_db, config, input) => {
      avatarCalls.push({ config, input });
      return {
        group_id: 'pg_character_81',
        asset_id: 'pa_character_81',
        asset_url: 'asset://pa_character_81',
        status: 'active',
      };
    },
  });

  assert.deepEqual(result, { task_id: 'tsk_private_avatar', status: 'queued' });
  assert.equal(avatarCalls.length, 1);
  assert.equal(avatarCalls[0].config.id, 70);
  assert.equal(avatarCalls[0].input.dramaId, 39);
  assert.equal(avatarCalls[0].input.sourceKind, 'image_generation');
  assert.equal(avatarCalls[0].input.sourceId, 81);
  assertSignedProviderAsset(
    avatarCalls[0].input.sourceUrl,
    '/static/projects/0039/ai-character.png',
    process.env.PLATFORM_JWT_SECRET
  );
  assert.equal(generationCalls.length, 1);
  assert.deepEqual(generationCalls[0].body.image_with_roles, [
    { url: 'asset://pa_character_81', role: 'reference_image' },
  ]);
});
